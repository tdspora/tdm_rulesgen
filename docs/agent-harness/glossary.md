...
| **[[LiteLLM backend]]** | The in-process [[LLM gateway]] backend that talks to OpenAI, Anthropic, Gemini, and Azure through one client interface. Databricks is **not** routed through this backend — see [[Databricks gateway]]. |
| **[[Databricks gateway]]** | Dedicated [[LLM gateway]] backend (`DatabricksOpenAIGatewayClient`) that uses `databricks_openai.DatabricksOpenAI` — Databricks' official OpenAI-compatible client. Selected when `llm_provider="databricks"` (or auto-detected via `DATABRICKS_RUNTIME_VERSION` / `DATABRICKS_HOST`). Authentication is fully handled by the client: inside DBR it picks up runtime credentials automatically; outside DBR it consumes the Databricks SDK auth chain (PAT, OAuth, service-principal, Azure AD). **No `OPENAI_API_KEY` or other provider key is required**. Requires the `[databricks]` optional extra (`pip install rulesgen[databricks]`, which pulls `databricks-openai`). |
| **[[completion parameters]]** | Knobs passed to the chat-completion call (`temperature`, `max_tokens`, `top_p`, `reasoning_effort`, …). Not every model accepts every knob — reasoning-tier models like Claude Opus 4.7 or OpenAI's o-series reject `temperature` or require omission of `max_tokens`. To accommodate: `RULESGEN_LLM_TEMPERATURE` accepts `null` / empty string to **omit the temperature parameter** entirely, and `RULESGEN_LLM_EXTRA_COMPLETION_PARAMS` is a JSON dict whose entries are merged into the completion call (e.g. `'{"max_tokens": 4096}'`). Applies uniformly to both the [[LiteLLM backend]] and the [[Databricks gateway]]. |
| **[[LLM provider auto-detection]]** | When `llm_provider="auto"`, the container detects which credential set is available and selects the matching gateway. Databricks is detected by `DATABRICKS_RUNTIME_VERSION` (set automatically inside any DBR job/notebook) or `DATABRICKS_HOST`, and — if `databricks_openai` is importable — routes through the [[Databricks gateway]]. OpenAI / Anthropic / Gemini / Azure are detected by their respective `*_API_KEY` env vars and routed through the [[LiteLLM backend]]. If nothing resolves, the gateway falls back to the [[stub backend]]. |
| **[[Databricks Foundation Model APIs]]** | Databricks-hosted, governance-controlled inference endpoints (e.g. `databricks-claude-sonnet-4-5`, `databricks-meta-llama-3-1-70b-instruct`) served at `https://<workspace>/serving-endpoints`. Reached via the [[Databricks gateway]]. |
...
| **[[guardrails]]** | Pre-LLM input scanners that block [[prompt injection]] / [[jailbreak]] attempts on [[untrusted rule input]] before it reaches the [[LLM gateway]]. Implemented in `src/rulesgen/infra/guardrails.py`. Default backend is the [[heuristic scanner]]; opt-in [[LLM Guard scanner]] via the `[guardrails]` extra. A block raises `GuardrailBlocked` and the API responds with a `guardrail_blocked` [[Problem Details]] document. |
| **[[prompt injection]]** | An attack class in which [[untrusted rule input]] tries to override system instructions, leak the system prompt, switch persona, or otherwise subvert the [[LLM gateway]]. Detected by [[guardrails]]. |
| **[[jailbreak]]** | A subclass of [[prompt injection]] aimed at unlocking unrestricted model behavior (`DAN mode`, `developer mode`, persona override). Same detection path as [[prompt injection]]. |
| **[[guardrail-verdict]]** | Structured result of one [[guardrails]] scan: `blocked`, `risk_score`, `scanner`, `categories`, `detail`. Categories and scores are written to the [[prompt audit]] but never echoed to API callers. |
| **[[heuristic scanner]]** | Default, zero-dependency [[guardrails]] backend. Word-boundary regex over known injection / jailbreak / role-override / code-escape / delimiter-injection patterns. Always-on; no model downloads. |
| **[[LLM Guard scanner]]** | Optional [[guardrails]] backend backed by `llm-guard`'s DeBERTa-v3 [[prompt injection]] classifier. Enabled by installing the `[guardrails]` (or `[guardrails-onnx]`) extra and setting `RULESGEN_GUARDRAILS_BACKEND=llm_guard`. Model id (`RULESGEN_GUARDRAILS_MODEL_ID`) can point at any compatible HF identifier *or* a local path (e.g. a [[Unity Catalog]] volume), and the model cache directory is configurable so HuggingFace egress can be customer-controlled. |
| **[[HTTP guardrail scanner]]** | Optional [[guardrails]] backend that POSTs each candidate prompt to a customer-owned classification endpoint and decides based on its score. Enabled by setting `RULESGEN_GUARDRAILS_BACKEND=http` + `RULESGEN_GUARDRAILS_HTTP_ENDPOINT`. Designed for [[Databricks Model Serving]] (request shape: `{"dataframe_records": [{"text": "..."}]}`, response shape configurable). Auth modes: `none`, `bearer` (env-var-named PAT), or `databricks_sdk` (resolves OAuth / service-principal credentials via `databricks-sdk`, requires the `[databricks]` extra). |
| **[[Databricks Model Serving]]** | Databricks-hosted inference endpoint over a UC-registered model. Recommended deployment target for the [[HTTP guardrail scanner]] when the customer's MRM requires a certified, governance-controlled classifier (e.g. Llama Guard 3 or a private fine-tune). |
| **[[Unity Catalog]]** | Databricks governance plane that owns model registry + volumes. A UC volume path like `/Volumes/<catalog>/<schema>/<volume>/hf-cache` is the canonical place to stage an approved HF classifier for the [[LLM Guard scanner]] in air-gapped deployments. |
...
| **[[agent (subagent)]]** | A focused role with its own model, tools, and prompt under `.claude/agents/`. Today: `rulesgen-architect`, `code-reviewer`, `security-reviewer`, `codebase-researcher`, `implementation-engineer`, `test-engineer`, `integration-test-engineer`, `release-engineer`, `technical-writer`, `docs-reviewer`. |
| **[[technical writer (agent)]]** | The `rulesgen-technical-writer` agent. Authors and updates public docs, design docs, contributor docs, sample READMEs, and the glossary. Tools: read + write. Produces [[testable code fence]] content and resolves links before handoff. Pairs with the [[docs reviewer]]. |
| **[[docs reviewer]]** | The `rulesgen-docs-reviewer` agent. Read-only reviewer that checks doc changes for correctness, [[testable code fence]] discipline, glossary alignment, link integrity, and [[secret hygiene]]. Produces BLOCKER / HIGH / MEDIUM-LOW findings. |
| **[[testable code fence]]** | A fenced Markdown code block whose test mode is explicit: `python` fences execute under [[sybil doc-test]], `bash` / `sh` fences declare expected output or carry `<!-- skip: next -->` (sybil directive; reason goes in a separate HTML comment above), `http` / `json` fences mirror a [[Pydantic v2]] schema or [[Problem Details]] body, and `dsl` fences parse + validate via the [[Library API]]. Rejected DSL examples use ` ```dsl !rejected ` with the expected error class captured by an `<!-- expects: <ExceptionClassName> -->` comment on the next line. |
| **[[sybil doc-test]]** | The `sybil`-based collector at `tests/contract/test_docs_fences.py` that turns every Python [[testable code fence]] into a pytest item. Adding `sybil` is an [[escalation trigger]] (dependency change). |
| **[[doc-test]]** | The broader contract suite under `tests/contract/test_docs_*.py` that asserts every documented Settings field, schema field, endpoint, glossary term, internal link, and DSL example matches the current source. |
